...
namespace DFLServer
{
    public static class BlackListManager
    {
        ...
        public static void Initialize()
        {
            ...
            if (!File.Exists(Server.config.BlackListFilePath))
            {
                IPBlackList = new HashSet<string>();
                File.WriteAllText(Server.config.BlackListFilePath, JsonConvert.SerializeObject(IPBlackList));
            }
            IPBlackList = JsonConvert.DeserializeObject<HashSet<string>>(File.ReadAllText(Server.config.BlackListFilePath));
