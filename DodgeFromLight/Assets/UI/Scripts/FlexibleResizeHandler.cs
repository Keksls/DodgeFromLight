...
public class FlexibleResizeHandler : MonoBehaviour
{
    ...
    void OnDrag(BaseEventData data)
    {
        ...
        if (horizontalEdge != null)
        {
            ...
            {
                float max = Mathf.Min(MaximumDimmensions.x, (Target.position.x * (1f / Target.pivot.x)) - Padding.x);
                float newWidth = Mathf.Clamp(Target.sizeDelta.x + ped.delta.x, MinimumDimmensions.x, max);
                float deltaPosX = (newWidth - Target.sizeDelta.x) * Target.pivot.x;
        ...
        if (verticalEdge != null)
        {
            ...
            {
                float max = Mathf.Min(MaximumDimmensions.y, (Target.position.y * (1f / Target.pivot.y)) - Padding.y);
                float newHeight = Mathf.Clamp(Target.sizeDelta.y - ped.delta.y, MinimumDimmensions.y, max);
