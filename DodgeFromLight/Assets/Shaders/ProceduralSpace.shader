...
Shader "ProceduralSpace"
{
	Properties
	{
		...
		_Contrast("Contrast", Range(0.001 , 1)) = 1
		...
		_Sunsize("Sunsize", Range(0 , 1)) = 0.2
		_SunIntensity("SunIntensity", Range(0.25, 25)) = 1
		_SunColor("SunColor", Color) = (0.928674,1,0,0)
		_SunUp("SunUp", Range(-1, 1)) = 0.2
		_SunHorizontal("SunHorirontal", Range(0, 1)) = 1
	}
	...
	SubShader
	{
		...
		Pass
		{
			...
			uniform float _SunHorizontal;
			uniform float _SunUp;
			...
			float snoise( float3 v )
			{
				...
				return 42.0 * dot( m, px) * _Contrast;
			}
			...
			v2f vert ( appdata v )
			{
				...
				o.ase_texcoord = v.vertex * _Scale;
			...
			fixed4 frag (v2f i ) : SV_Target
			{
				...
				fixed4 finalColor;
				_Seed = _Seed + (_Time.y * _AnimationSpeed * (1 + _Scale));
				...
				float4 color249 = float4(0,0,0,0);
				...
				float simplePerlin3D105 = snoise( ( Position79 * 1 ) );
				...
				float4 color259 = float4(0,0,0,0);
				...
				float lerpResult315 = lerp( 0.5 , 0.99999999, temp_output_340_0);
				float3 ase_worldPos = i.ase_texcoord1.xyz;
				float3 worldSpaceLightDir = Unity_SafeNormalize(float3(1, _SunUp * 2, _SunHorizontal * 2));
				...
				_SunColor = _SunColor * _SunIntensity;
				float4 lerpResult301 = lerp( ( ( (( simplePerlin3D42 + simplePerlin3D44 )*0.1 + _DustAmount) * 0.1 ) + lerpResult248 + lerpResult258 ) , _SunColor , ( smoothstepResult299 + ( smoothstepResult330 * ( simplePerlin3D323 * 0.2 ) ) ));
				...
				finalColor = (lerpResult301 + ((smoothstepResult309 * lerpResult320) * (_SunColor / 3.0)));
				return finalColor;
			}
			ENDCG
		}
